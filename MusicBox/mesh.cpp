...
Mesh::Mesh(string filename, GLuint type0) {
...
void Mesh::load(string filename, GLuint type) {
	// Release resources
	release();
	...
	vector<vec2> raw_texcoords;
	vector<vec3> raw_normals;
	vector<unsigned int> v_elements;
	vector<unsigned int> tc_elements;
	...
	while (getline(file, line)) {
		if (line.substr(0, 2) == "v ") {
			...
			int index2 = lastIndexOfNumberLetter(line);
			vector<string> values = split(line.substr(index1, index2 - index1 + 1), ' ');
			raw_texcoords.push_back(vec2(stof(values[0]), stof(values[1])));
			...
			for (int i = 0; i < values.size() - 2; i++) {
				...
				if (v1.size() >= 3 && v1[1].length() > 0) {
					tc_elements.push_back(stoul(v1[1]) - 1);
					tc_elements.push_back(stoul(v2[1]) - 1);
					tc_elements.push_back(stoul(v3[1]) - 1);
				}
			}
		} 
	}
	...
	// Create vertex array
	vertices = vector<Vtx>(v_elements.size());
	for (int i = 0; i < v_elements.size(); i += 3) {
		// Store type
		vertices[i + 0].type = type;
		vertices[i + 1].type = type;
		vertices[i + 2].type = type;
		...
		if (tc_elements.size() > 0) {
			vertices[i + 0].tc = raw_texcoords[tc_elements[i + 0]];
			vertices[i + 1].tc = raw_texcoords[tc_elements[i + 1]];
			vertices[i + 2].tc = raw_texcoords[tc_elements[i + 2]];
		}
	...
	glVertexAttribIPointer(3, 1, GL_UNSIGNED_INT, sizeof(Vtx), (GLvoid*)(sizeof(vec3) + sizeof(vec3) + sizeof(vec2)));
...
// Fix bouding box
mat4 Mesh::fixBB() {
	pair<vec3, vec3> meshBB = boundingBox();
	mat4 fixMat = scale(mat4(1.f), vec3(1.f / length(meshBB.second - meshBB.first)));
	fixMat = translate(fixMat, vec3(-(meshBB.first + meshBB.second) / 2.f));
	return fixMat;
}
